 ...
 */
const fetchFontSource = async (source) => {
    ...
    return [
        {
            ...
            data: await fetchFontSource("https://api.fontsource.org/v1/fonts/atkinson-hyperlegible/latin-400-normal.ttf"),
            weight: 400,
            style: "normal",
        },
        {
            name: "Atkinson Hyperlegible",
            data: await fetchFontSource("https://api.fontsource.org/v1/fonts/atkinson-hyperlegible/latin-400-italic.ttf"),
            weight: 400,
            style: "italic",
        },
        {
            name: "Atkinson Hyperlegible",
            data: await fetchFontSource("https://api.fontsource.org/v1/fonts/atkinson-hyperlegible/latin-700-normal.ttf"),
            weight: 700,
            style: "normal",
        },
        {
            name: "Atkinson Hyperlegible",
            data: await fetchFontSource("https://api.fontsource.org/v1/fonts/atkinson-hyperlegible/latin-700-italic.ttf"),
            weight: 700,
            style: "italic",
        },
    ];
...
export const getFonts = async (options) => {
    ...
    const fonts = await Promise.all(options.fonts.map(async (font) => {
        try {
            let fontItem = {
                name: font.name,
            ...
            if (font.weight)
                fontItem.weight = font.weight;
            if (font.style)
                fontItem.style = font.style;
            if (font.lang)
                fontItem.lang = font.lang;
            return fontItem;
        }
        catch (e) {
            console.error(cyan("  │  ") + red("✖") + dim(` Font not found: ${font.source}`));
            return undefined;
        }
    }));
    const defaults = await getDefaultFonts();
    fonts.push(...defaults);
    return fonts.filter((font) => font !== undefined);
};
/**
 ...
 */
export const getTemplate = (options, template) => {
    switch (template) {
        case "bgPhoto":
 ...
 */
const getImageBuffer = async (path) => {
    let buffer;
 ...
 */
export const loadImageToBase64 = async (path) => {
    const extRe = /(?:\.([^.]+))?$/;
    const ext = extRe.exec(path)?.[1];
    const supported = ["jpg", "jpeg", "png", "svg"];
    if (!ext || supported.indexOf(ext) < 0)
        return null;
    const isSvg = ext === "svg";
    ...
    try {
        ...
        console.error(cyan("  │  ") + red("✖") + dim(` File not found: ${path}`));
        return null;
    }
